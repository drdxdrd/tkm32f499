...
{	
	RCC_DeInit();
	//HSE on
	//CR锟侥达拷锟斤拷BIT16位锟斤拷HSEON位锟斤拷锟斤拷1锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟解部时锟斤拷HSE锟斤拷为系统时锟斤拷
	...
	RCC_PLLCmd(ENABLE);
	RCC_SYSCLKConfig(RCC_SYSCLKSource_PLLCLK);//选锟斤拷锟解部时锟斤拷锟斤拷为系统时锟斤拷
}
/**
...
*   Once the RTC clock is selected it can锟絫 be changed unless the
...
/**
* @brief  Clears the RCC锟絪 interrupt pending bits.
